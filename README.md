...
* **Calendar Sync:** Automatically unlocks doors for events matching specific keywords (e.g., `*` or `Meeting`).
...
Before installing, ensure you have the following running in Home Assistant:

1.  **UniFi Access Integration:** Your doors (`lock.door_x`) and rules (`select.door_x_rule`) must be visible in Home Assistant.
2.  **HACS (Home Assistant Community Store):** Required to install Pyscript.
3.  **Pyscript Integration:** This automation runs on Python. Standard YAML automations will not work.
...
### Step 2: Create Dashboard Helpers
...
1.  **Entity IDs:** Change `lock.door_0664` and `select.door_0664_door_lock_rule` to match your actual UniFi device names.
